...
if (length(args) != 1)
    stop("Usage: Rscript NCBIdataPrep.R  <file.soft>")

fileName <- basename(args[1])          # "GDS531_full.soft"
path     <- dirname(normalizePath(args[1]))

#Find the beginning of the data
all_lines <- readLines(file.path(path, fileName), warn = FALSE)

beg <- which(grepl("!dataset_table_begin", all_lines))[1] + 1
end <- which(grepl("!dataset_table_end",   all_lines))[1] - 1
if (is.na(beg) || is.na(end))
    stop("Could not locate !dataset_table markers in the .soft file")
...
data <- fread(
  text   = paste(all_lines[beg:end], collapse = "\n"),
  sep    = "\t",
  header = FALSE,
  quote  = "",
  data.table = FALSE,
